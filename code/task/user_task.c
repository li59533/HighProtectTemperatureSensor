...
void UserTask_Init(uint8_t taskId)
{
	...
	OS_Timer_Start(g_UserTask_Id, USER_TASK_LOOP2_EVENT,500);	

	BSP_LED_Blink( BSP_LED_TEST , 0 , 10, 1000);

	BSP_LMT01_Power_ON();

	UserTask_Send_Event(USER_TASK_LMT01_LOOP_EVENT);	
}
...
osal_event_t UserTask_Process(uint8_t taskid,osal_event_t events)
{
    ...
    if (events & USER_TASK_LOOP2_EVENT)
    {
		BSP_ShowDate();
		BSP_LMT01_TestFunc();
		APP_Battery_TestFunc();
		OS_Timer_Start(g_UserTask_Id, USER_TASK_LOOP2_EVENT,1000);			
        return events ^ USER_TASK_LOOP2_EVENT;
    }	

    if (events & USER_TASK_LMT01_LOOP_EVENT)
    {
		BSP_LMT01_CoreLoop(); 
		OS_Timer_Start(g_UserTask_Id, USER_TASK_LMT01_LOOP_EVENT,20);			
        return events ^ USER_TASK_LMT01_LOOP_EVENT;
    }	
